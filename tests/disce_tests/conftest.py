...
class HTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def __init__(
        ...
        super().__init__(request, client_address, server, directory=str(HTTPRequestHandler.DIRECTORY))


@pytest.fixture(scope="session")
def server_url() -> Generator[str]:
    host = "127.0.0.1"
    server = HTTPServer((host, 0), HTTPRequestHandler)
...
@pytest.fixture(scope="session")
def geckodriver_path(pytestconfig: pytest.Config) -> Path:
...
@pytest.fixture(scope="session")
def session_browser(geckodriver_path: Path) -> Generator[Firefox]:
    options = Options()
    options.add_argument("--headless")
    options.set_preference("devtools.console.stdout.content", value=True)
    with _forward_to_stderr() as forwarded_stderr:
        service = FirefoxService(executable_path=str(geckodriver_path), log_output=forwarded_stderr)
...
@pytest.fixture
def browser(
    session_browser: Firefox, server_url: str, request: pytest.FixtureRequest, capsys: pytest.CaptureFixture[str]
) -> Firefox:
    ...
    browser.execute_script("pyodide = await loadPyodide();")
    return browser
